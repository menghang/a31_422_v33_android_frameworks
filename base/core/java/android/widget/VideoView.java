...
public class VideoView extends SurfaceView implements MediaPlayerControl {
    ...
    private void openVideo() {
        ...
        try {
			...
			/*
			if (mUri.toString().startsWith("file:///storage")) {
				String mString = mUri.toString();
				mString = mString.replaceFirst("storage", "mnt/shell");
				mUri = Uri.parse(mString);
				Log.d(TAG,"replace uri!!!!!!!!" + mUri.getPath());
			} else if (mUri.toString().startsWith("/storage")) {
				String mString = mUri.toString();
				mString = mString.replaceFirst("storage", "mnt/shell");
				mUri = Uri.parse(mString);
				Log.d(TAG,"replace uri!!!!!!!!" + mUri.getPath());
			...
			*/
			...
			if(mSrtList!= null && mSrtList.size() > 0)
			{
				...
				{
					/*
					String mString = mSrtList.get(j);
					if (mString.startsWith("/storage")) {
						mString = mString.replaceFirst("storage", "mnt/shell");
            ...
            { 
                ...
                { 
					...
					{
						Log.v("fuqiang","select MEDIA_TRACK_TYPE_TIMEDTEXT ~~~~~~~~~~~~~");
						mMediaPlayer.selectTrack(j);
						break;
					}
                } 
