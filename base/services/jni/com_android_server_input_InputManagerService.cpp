...
/* add by cjcheng start {----------------------------------- */
/* support the mouse mode 2013-12-03 */
static void nativeKeyEnterMouseMode(JNIEnv* env, jclass clazz, jint ptr) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
    im->getInputManager()->getReader()->keyEnterMouseMode();
}

static void nativeKeyExitMouseMode(JNIEnv* env, jclass clazz, jint ptr) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
    im->getInputManager()->getReader()->keyExitMouseMode();
}

static void nativeKeySetMouseDistance(JNIEnv* env, jclass clazz, jint ptr, jint distance) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
    im->getInputManager()->getReader()->keySetMouseDistance((int)distance);
}

static void nativeKeySetMouseBtnCode(JNIEnv* env, jclass clazz, jint ptr,jint leftbtn,jint midbtn,jint rightbtn) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
    im->getInputManager()->getReader()->keySetMouseBtnCode((int)leftbtn,(int)midbtn,(int)rightbtn);
}

static void nativeKeySetMouseMoveCode(JNIEnv* env, jclass clazz, jint ptr, jint left,jint right,jint top,jint bottom) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static JNINativeMethod gInputManagerMethods[] = {
    ...
    { "nativeKeyEnterMouseMode", "(I)V",
        (void*) nativeKeyEnterMouseMode},
    { "nativeKeyExitMouseMode", "(I)V",
        (void*) nativeKeyExitMouseMode},
    { "nativeKeySetMouseDistance", "(II)V",
        (void*) nativeKeySetMouseDistance},
    { "nativeKeySetMouseBtnCode", "(IIII)V",
        (void*) nativeKeySetMouseBtnCode},
    { "nativeKeySetMouseMoveCode", "(IIIII)V",
        (void*) nativeKeySetMouseMoveCode},
    /* add by cjcheng end   -----------------------------------} */
};
