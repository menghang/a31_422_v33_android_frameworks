...
InputReader::InputReader(const sp<EventHubInterface>& eventHub,
    ...
    { // acquire lock
        ...
        /* support the mouse mode 2013-12-03 */
        ...
        if (type < EventHubInterface::FIRST_SYNTHETIC_EVENT) {
            ...
            case EventHubInterface::DEVICE_ADDED:
                ...
                if(mVirtualMouseCreated == false) {
                    addVirtualMouseDevice(rawEvent->when, mMouseDeviceId);
...
void InputReader::addVirtualMouseDevice(nsecs_t when, int32_t deviceId) {
    char devicename[80] = "Virtual Mouse Device";
    String8 name = String8(devicename);
    ssize_t deviceIndex = mDevices.indexOfKey(deviceId);

    if (deviceIndex >= 0) {
        ALOGW("cjcheng Ignoring VirtualMouse device added event for deviceId %d.", deviceId);
        return;
    }
    ...
    classes = INPUT_DEVICE_CLASS_EXTERNAL | INPUT_DEVICE_CLASS_CURSOR;
    InputDevice* device = createVirtualMouse(identifier,classes);
    device->configure(when, &mConfig, 0);
    device->reset(when);

    if (device->isIgnored()) {
        ALOGI("cjcheng Device added: id=%d, name='%s' (ignored non-input device)", deviceId,
                identifier.name.string());
    } else {
        ALOGI("cjcheng Device added: id=0x%x, name='%s', sources=0x%08x", deviceId,
...
#if DEBUG_RAW_EVENTS
        ALOGD("Input event: device=%d type=0x%04x scancode=0x%04x, value=0x%08x",
...
/*
        ALOGD("count = %d, rawEvent->type = %d\n",count,rawEvent->type);
        ALOGD("tmpRawEvent->deviceId = %x,mMouseDeviceId = %x, mDistance = %d\n",tmpRawEvent->deviceId,mMouseDeviceId, mDistance);
        ALOGD("mLeft = %d,mRight = %d,mKeyInMouseMode = %d\n",mLeft,mRight,mKeyInMouseMode);
        ALOGD("mTop = %d,mBottom = %d\n",mTop,mBottom);
*/
        if(mKeyInMouseMode) {
...
void InputReader::keyEnterMouseMode()
{
    ...
    if (deviceIndex < 0) {
        ALOGW("Discarding event for unknown mMouseDeviceId %d.", mMouseDeviceId);
        return;
    }

    InputDevice* device = mDevices.valueAt(deviceIndex);
    if (device->isIgnored()) {
        ALOGD("Discarding event for ignored mMouseDeviceId %d.", mMouseDeviceId);
        return;
    }
...
uint32_t CursorButtonAccumulator::getButtonState() const {
    ...
    if (mBtnRight) {
        ...
        result |= AMOTION_EVENT_BUTTON_BACK; //add by Lynx 20130402
    }
    if (mBtnMiddle) {
