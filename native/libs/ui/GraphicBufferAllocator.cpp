...
void GraphicBufferAllocator::dump(String8& result) const
{
    ...
    for (size_t i=0 ; i<c ; i++) {
        ...
        else
        {
            if(rec.format == 0x1ff)
            {
                buffer_size = rec.s * rec.h * 4;
            }
            else if(rec.format == HAL_PIXEL_FORMAT_AW_NV12)
            {
                buffer_size = rec.s * rec.h * 3 / 2;
            }
            total += buffer_size;
        }
    }

    snprintf(buffer, SIZE, "Allocated %d buffers, Total allocated:%.2f KB:\n", c, total/1024.0f);
    ...
    for (size_t i=0 ; i<c ; i++) {
        ...
        if (rec.size) {
            ...
            snprintf(buffer, SIZE, "%10p: %7.2f KiB | %4u (%4u) x %4u | %8X | 0x%08x\n",
                    list.keyAt(i), buffer_size/1024.0f,
                    rec.w, rec.s, rec.h, rec.format, rec.usage);
        }
        result.append(buffer);
    }
