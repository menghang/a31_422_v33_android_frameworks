        ...
        uint32_t minLayerZ, uint32_t maxLayerZ)
{
    ...
    switch (atoi(property)) {
        ...
        case 270:
            if ((sw > hw_h) || (sh > hw_w)) {
                ALOGE("size mismatch (%d, %d) > (%d, %d)", sw, sh, hw_h, hw_w);
                return BAD_VALUE;
            }
            sw = (!sw) ? hw_h : sw;
            sh = (!sh) ? hw_w : sh;
...
//            sw, sh, minLayerZ, maxLayerZ);
    ...
    if (status == GL_FRAMEBUFFER_COMPLETE_OES) {
        ...
        switch (atoi(property)) {
            case 90:
                glOrthof(0, hw_h, hw_w, 0, 0, 1);
                glMatrixMode(GL_MODELVIEW);
                glPushMatrix();
                glRotatef(90.0f, 0.0f, 0.0f, 1.0f);
                glTranslatef(0.0f, -(GLfloat)hw_h, 0.0f);
                break;
            case 180:
                ...
                glTranslatef(-(GLfloat)hw_w, -(GLfloat)hw_h, 0.0f);
                break;
            case 270:
                glOrthof(0, hw_h, hw_w, 0, 0, 1);
                glMatrixMode(GL_MODELVIEW);
                ...
                glTranslatef(-(GLfloat)hw_w, 0.0f, 0.0f);
                break;
            case 0:
            default:
                glOrthof(0, hw_w, hw_h, 0, 0, 1);
